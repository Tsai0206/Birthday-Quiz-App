-- ==========================================
-- 🎮 Birthday Quiz Game - Complete Database Schema (Fixed)
-- ==========================================
...
-- Delete duplicate answers (keep only the first one per player per question)
DELETE FROM answers a
WHERE a.id NOT IN (
  SELECT MIN(id)
  FROM answers
  GROUP BY player_id, question_index
);
...
-- Add unique constraint for one answer per player per question (AFTER cleaning duplicates)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- ==========================================
-- STEP 5: OPTION_SHUFFLES TABLE (Anti-Cheating)
-- ==========================================
...
-- 2. You should now see 4 tables: games, players, answers, option_shuffles
